...
- 플레이어/적 HP 텍스트 표시
- Battle Guide 조작 힌트 UI: Attack, Fire Skill/Burn, Guard, Enemy Intent, Retry 흐름 안내
- 플레이어/적 HP 바 슬라이더 표시
- 플레이어 AP 텍스트/바 슬라이더 표시 및 턴 시작 시 AP 회복
...
- Victory/Defeat 후 결과/턴 수, 최종 HP/AP, 피해량, 선택 횟수, 클리어 속도/생존율, 전투 랭크/보상 골드, 플레이 팁, 마지막 적 패턴을 compact하게 보여주는 결과 요약 UI
...
4. 상단 가운데 `▶ Play` 버튼 클릭
5. 상단의 `Battle Guide` 라벨에서 `Attack`, `Fire Skill`, `Guard`, `Enemy Intent`, `Retry` 흐름을 먼저 확인
6. `Attack`, `Fire Skill`, `Guard`, `End Turn` 버튼으로 전투 테스트
7. 공격/방어 후 플레이어/적 HP 바가 HP 텍스트와 함께 줄어드는지 확인
8. `Guard`를 누르면 플레이어 쪽 상태가 `Status: Guarding`으로 바뀌고, 적 공격을 받은 뒤 `Status: Ready`로 돌아오는지 확인
9. 적 HP 아래 `Next Enemy: Normal Attack (15)` 또는 `Next Enemy: Heavy Slam (30)` 의도 표시가 갱신되는지 확인
10. `Fire Skill` 사용 후 플레이어 AP 바가 `3/3 -> 1/3`처럼 줄어드는지 확인
11. 화면 하단 `Recent Actions` 패널에 플레이어 턴, Fire Skill, Guard, 적 공격 같은 최근 행동이 누적되는지 확인
12. Victory/Defeat 후 결과 요약 UI에 `Damage: dealt ..., taken ...` / `Choices: Guard ..., Skills ...` / `Pace: ... | Survival: ...` / `Rank: ... | Reward: ...G`처럼 묶인 compact 문구와 `Tip`이 함께 나타나고, 뒤에 어두운 패널이 표시되는지 확인
13. 나타나는 `Retry` 버튼으로 전투 재시작 테스트
14. 재시작 후 결과 요약 UI가 사라지고 HP/AP 바와 플레이어 상태, 전투 로그가 초기 상태로 돌아오는지 확인
...
- 데이터 기반으로 캐릭터/스킬/적 패턴 값을 분리하려는 구조
- `PlayerTurn -> EnemyTurn -> Victory/Defeat -> Retry` 형태의 명확한 전투 상태 흐름
- AP, 상태 이상, 방어, 일반 공격/강공격 적 패턴 등 턴제 RPG의 기본 전략 요소
- Enemy Intent UI로 플레이어가 다음 적 공격을 보고 공격/방어 선택을 고민하게 만드는 UX
- Battle Guide 라벨로 처음 보는 플레이어가 Attack, Fire Skill/Burn, Guard, Enemy Intent, Retry 흐름을 바로 알 수 있게 만든 온보딩 UX
- Player Status UI로 `Guarding` 상태를 즉시 보여줘 방어가 적용 중인지 명확히 전달하는 UX
- HP/AP 텍스트와 리소스 바를 함께 갱신해 전투 상태를 빠르게 읽을 수 있게 만든 UX
- `Recent Actions` 전투 로그 패널로 최근 행동 흐름을 별도 영역에서 따라갈 수 있게 만든 UX
- 결과 요약 UI로 플레이어가 전투 후 최종 상태, 누적 피해량, 방어/스킬 사용 횟수, 클리어 속도/생존율, 전투 랭크/보상을 짧게 묶어 읽고 플레이 팁과 적 패턴을 되돌아볼 수 있는 UX
