...
- Run Status UI: `Stage 1 In Progress`, `Encounter Clear - Continue`, `Retry Current Encounter`, `Stage 1 Complete`로 전체 진행 상태 안내
...
6. 상단의 `Battle Guide` 라벨에서 `Attack`, `Fire Skill`, `Guard`, `Enemy Intent`, `Continue`, `Retry` 흐름을 먼저 확인
...
15. 첫 Victory 후 목표 라벨이 `Objective Complete: Stage 1-1: Slime Scout | Continue to next encounter`로, 진행도 라벨이 `Progress: Encounter 1/2 | Encounter Clear`로 바뀌는지 확인
...
17. boss Victory 후 메시지가 `Final Clear! Stage 1 completed.`로 바뀌고 목표/진행도가 `Objective Complete: Stage 1 cleared`, `Progress: Encounter 2/2 | Stage Clear`로 바뀌며 `Continue`는 숨겨지는지 확인
...
- Stage 1-1 일반 encounter에서 Stage 1-2 boss encounter로 이어지는 작은 vertical slice 진행 구조와 현재 목표/완료 상태/진행도를 분리해 보여주는 Run Status + Objective + Progress UI
- AP, 상태 이상, 방어, 일반 공격/강공격 적 패턴 등 턴제 RPG의 기본 전략 요소
- Enemy Intent UI로 플레이어가 다음 적 공격을 보고 공격/방어 선택을 고민하게 만드는 UX
- Run Status 라벨로 현재 stage run이 진행 중인지, encounter clear인지, retry가 필요한지, 최종 clear인지 즉시 알 수 있게 만든 상단 진행 UX
