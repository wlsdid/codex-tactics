...
- Stage UI: 현재 encounter를 `Stage 1-1: Slime Scout`처럼 표시
...
6. 현재 스테이지 라벨이 `Stage 1-1: Slime Scout`로 표시되는지 확인
...
13. Victory/Defeat 후 결과 요약 UI에 `Damage: dealt ..., taken ...` / `Choices: Guard ..., Skills ...` / `Pace: ... | Survival: ...` / `Rank: ... | Reward: ...G`처럼 묶인 compact 문구와 `Tip`이 함께 나타나고, 뒤에 어두운 패널이 표시되는지 확인
14. 첫 Victory 후 `Continue` 버튼을 눌러 `Stage 1-2: Slime King` boss encounter로 넘어가는지 확인
15. boss Victory 후 메시지가 `Final Clear! Stage 1 completed.`로 바뀌고 `Continue`는 숨겨지는지 확인
16. 나타나는 `Retry` 버튼으로 현재 encounter 재시작 테스트
17. 재시작 후 결과 요약 UI가 사라지고 HP/AP 바와 플레이어 상태, 전투 로그가 초기 상태로 돌아오는지 확인
...
- Stage 1-1 일반 encounter에서 Stage 1-2 boss encounter로 이어지는 작은 vertical slice 진행 구조
