...
- Stage UI: 현재 encounter를 `Stage 1-1: Slime Scout`처럼 표시하고, Objective/Progress 라벨로 목표와 진행도(`Encounter 1/2`)를 안내
...
6. 현재 스테이지 라벨이 `Stage 1-1: Slime Scout`로 표시되고 목표/진행도 라벨이 `Objective: Defeat Slime Scout`, `Progress: Encounter 1/2 | Active`로 표시되는지 확인
...
14. 첫 Victory 후 목표 라벨이 `Objective Complete: Stage 1-1: Slime Scout | Continue to next encounter`로, 진행도 라벨이 `Progress: Encounter 1/2 | Encounter Clear`로 바뀌는지 확인
15. `Continue` 버튼을 눌러 `Stage 1-2: Slime King` boss encounter로 넘어가고 목표/진행도가 `Objective: Defeat Slime King`, `Progress: Encounter 2/2 | Active`로 갱신되는지 확인
16. boss Victory 후 메시지가 `Final Clear! Stage 1 completed.`로 바뀌고 목표/진행도가 `Objective Complete: Stage 1 cleared`, `Progress: Encounter 2/2 | Stage Clear`로 바뀌며 `Continue`는 숨겨지는지 확인
...
- Stage 1-1 일반 encounter에서 Stage 1-2 boss encounter로 이어지는 작은 vertical slice 진행 구조와 현재 목표/완료 상태/진행도를 분리해 보여주는 Objective + Progress UI
