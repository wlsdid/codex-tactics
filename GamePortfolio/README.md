...
- Victory/Defeat 후 결과, 적 턴 수, 최종 HP/AP, 피해량, 방어/스킬 사용 횟수, 클리어 속도, 생존율, 전투 랭크, 보상 골드, 플레이 팁, 마지막 적 패턴을 보여주는 결과 요약 UI
...
10. Victory/Defeat 후 결과 요약 UI에 `Damage dealt` / `Damage taken` / `Guard uses` / `Skills used` / `Pace` / `Survival` / `Rank` / `Reward` / `Tip`이 함께 나타나고, 뒤에 어두운 패널이 표시되는지 확인
...
- 결과 요약 UI로 플레이어가 전투 후 최종 상태, 누적 피해량, 방어/스킬 사용 횟수, 클리어 속도, 남은 HP 생존율, 전투 랭크, 랭크 기반 보상 골드, 플레이 팁, 적 패턴을 되돌아볼 수 있는 UX
