...
- Victory/Defeat 후 결과, 적 턴 수, 최종 HP/AP, 피해량, 방어/스킬 사용 횟수, 클리어 속도, 전투 랭크, 보상 골드, 플레이 팁, 마지막 적 패턴을 보여주는 결과 요약 UI
...
4. 상단 가운데 `▶ Play` 버튼 클릭
5. `Attack`, `Fire Skill`, `Guard`, `End Turn` 버튼으로 전투 테스트
6. 공격/방어 후 플레이어/적 HP 바가 HP 텍스트와 함께 줄어드는지 확인
7. `Guard`를 누르면 플레이어 쪽 상태가 `Status: Guarding`으로 바뀌고, 적 공격을 받은 뒤 `Status: Ready`로 돌아오는지 확인
8. 적 HP 아래 `Next Enemy: Normal Attack (15)` 또는 `Next Enemy: Heavy Slam (30)` 의도 표시가 갱신되는지 확인
9. `Fire Skill` 사용 후 플레이어 AP 바가 `3/3 -> 1/3`처럼 줄어드는지 확인
10. Victory/Defeat 후 결과 요약 UI에 `Damage dealt` / `Damage taken` / `Guard uses` / `Skills used` / `Pace` / `Rank` / `Reward` / `Tip`이 함께 나타나고, 뒤에 어두운 패널이 표시되는지 확인
11. 나타나는 `Retry` 버튼으로 전투 재시작 테스트
12. 재시작 후 결과 요약 UI가 사라지고 HP/AP 바와 플레이어 상태가 초기화되는지 확인
...
- 결과 요약 UI로 플레이어가 전투 후 최종 상태, 누적 피해량, 방어/스킬 사용 횟수, 클리어 속도, 전투 랭크, 랭크 기반 보상 골드, 플레이 팁, 적 패턴을 되돌아볼 수 있는 UX
- `BattleResultData.cs`로 결과 요약 값을 모으고, `BattleResultEvaluator.cs`로 랭크/속도/보상/팁 규칙을 분리한 뒤, `BattleResultPresenter.cs`로 표시 문장 포맷을 분리해 결과 UI가 커져도 구조를 유지하기 쉽게 정리
