...
11. Victory/Defeat 후 결과 요약 UI에 `Damage: dealt ..., taken ...` / `Choices: Guard ..., Skills ...` / `Pace: ... | Survival: ...` / `Rank: ... | Reward: ...G`처럼 묶인 compact 문구와 `Tip`이 함께 나타나고, 뒤에 어두운 패널이 표시되는지 확인
12. 나타나는 `Retry` 버튼으로 전투 재시작 테스트
13. 재시작 후 결과 요약 UI가 사라지고 HP/AP 바와 플레이어 상태가 초기화되는지 확인
