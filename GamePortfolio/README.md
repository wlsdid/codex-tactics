...
- Stage UI: `Codex Tactics` 제목 아래 Run Status, 현재 encounter, Objective, Progress 라벨로 현재 진행 상태를 안내
- StageData/EnemyData 기반의 2단계 전투 구성: 일반 슬라임 encounter 후 슬라임 킹 boss encounter
- Run Status UI: `Stage 1 In Progress`, `Encounter Clear - Continue to Next`, `Retry Current Encounter`, `Final Clear - Stage 1 Complete`로 전체 진행 상태 안내
- Battle Guide 조작 힌트 UI: Attack, Fire Skill/Burn, Guard, Enemy Intent, Continue, Final Clear, Retry 흐름 안내
- 플레이어/적 HP 바 슬라이더 표시
...
- Victory 후 다음 encounter로 넘어가는 `Continue` 버튼: Stage 1-1 일반 슬라임에서 Stage 1-2 슬라임 킹으로 진행, 첫 clear 메시지는 다음 encounter 이름을 알려주고 마지막 승리 시 Final Clear 안내 표시
...
4. 상단 가운데 `▶ Play` 버튼 클릭
5. 상단 제목이 `Codex Tactics`로 보이고, `Run Status: Stage 1 In Progress` 라벨이 표시되는지 확인
6. 상단의 `Battle Guide` 라벨에서 `Attack`, `Fire Skill`, `Guard`, `Enemy Intent`, `Continue`, `Final Clear`, `Retry` 흐름을 먼저 확인
7. 현재 스테이지 라벨이 `Stage 1-1: Slime Scout`로 표시되고 목표/진행도 라벨이 `Objective: Defeat Slime Scout`, `Progress: Encounter 1/2 | Active`로 표시되는지 확인
8. `Attack`, `Fire Skill`, `Guard`, `End Turn` 버튼으로 전투 테스트
9. 공격/방어 후 플레이어/적 HP 바가 HP 텍스트와 함께 줄어들고, 괄호 안 퍼센트도 함께 바뀌는지 확인
10. `Guard`를 누르면 플레이어 쪽 상태가 `Status: Guarding`으로 바뀌고, 적 공격을 받은 뒤 `Status: Ready`로 돌아오는지 확인
11. 적 HP 아래 `Next Enemy: Normal Attack (15)` 또는 `Next Enemy: Heavy Slam (30)` 의도 표시가 갱신되는지 확인
12. `Fire Skill` 사용 후 플레이어 AP 바가 `3/3 (100%) -> 1/3 (33%)`처럼 줄어드는지 확인
13. 화면 하단 `Recent Actions` 패널에 플레이어 턴, Fire Skill, Guard, 적 공격 같은 최근 행동이 누적되는지 확인
14. Victory/Defeat 후 결과 요약 UI에 `Damage: dealt ..., taken ...` / `Choices: Guard ..., Skills ...` / `Pace: ... | Survival: ...` / `Rank: ... | Reward: ...G | Total Gold: ...G`처럼 묶인 compact 문구와 `Tip`이 함께 나타나고, 뒤에 어두운 패널이 표시되는지 확인
15. 첫 Victory 후 목표 라벨이 `Objective Complete: Stage 1-1: Slime Scout | Continue to Stage 1-2: Slime King`로, 진행도 라벨이 `Progress: Encounter 1/2 | Encounter Clear`로 바뀌고 메시지가 다음 encounter 이름을 안내하는지 확인
16. `Continue` 버튼을 눌러 `Stage 1-2: Slime King` boss encounter로 넘어가고 목표/진행도가 `Objective: Defeat Slime King`, `Progress: Encounter 2/2 | Active`로 갱신되는지 확인
17. boss Victory 후 메시지가 `Final Clear! Stage 1 completed. Review Total Gold...` 흐름으로 바뀌고 목표/진행도가 `Objective Complete: Stage 1 cleared | Final Clear`, `Progress: Encounter 2/2 | Stage Clear`로 바뀌며 `Continue`는 숨겨지는지 확인
18. 나타나는 `Retry` 버튼으로 현재 encounter 재시작 테스트
19. 재시작 후 결과 요약 UI가 사라지고 HP/AP 바와 플레이어 상태, 전투 로그가 초기 상태로 돌아오는지 확인
...
- Stage 1-1 일반 encounter에서 Stage 1-2 boss encounter로 이어지는 작은 vertical slice 진행 구조와 현재 목표/완료 상태/다음 목적지/진행도를 분리해 보여주는 Run Status + Objective + Progress UI
- AP, 상태 이상, 방어, 일반 공격/강공격 적 패턴 등 턴제 RPG의 기본 전략 요소
- Enemy Intent UI로 플레이어가 다음 적 공격을 보고 공격/방어 선택을 고민하게 만드는 UX
- Run Status 라벨로 현재 stage run이 진행 중인지, encounter clear 후 다음 진행 대기인지, retry가 필요한지, 최종 clear인지 즉시 알 수 있게 만든 상단 진행 UX
- Battle Guide 라벨로 처음 보는 플레이어가 Attack, Fire Skill/Burn, Guard, Enemy Intent, Continue, Final Clear, Retry 흐름을 바로 알 수 있게 만든 온보딩 UX
